...
class Dataset:
    ...
    def status(self):
        ...
        def has_any(root, ext):
            # Recursive: several of these are built per sensor/source
            # (<stage>/<sensor>/<crop>/), so this must find a match in ANY of
            # those, not just a single hardcoded default — otherwise the status
            # pills read "todo" even after building on e.g. Registered (the
            # app's own default sensor elsewhere).
        ...
        return {
            "pcd": has_any(os.path.join(self.derived_dir, "point_clouds", "cropped"), ".pcd"),
            "gt": has_any(os.path.join(self.derived_dir, "labels", "scorable"), ".json"),
...
def reset_pipeline(ds, keep_videos=True):
    """Delete everything the pipeline has GENERATED for this dataset — `data/derived/`
    and `outputs/` — so it can be re-run truly end-to-end from just the raw download.

    Left untouched (not generated by a pipeline run, so not this function's business):
      - `data/raw/`        the untouched original download
      - `map/`             the optional HD-map data
      - `config/`          site geometry, lanes, georeference (curated calibration —
                           `site_geometry.json`/`lanes.geojson` CAN be edited via the
                           app, but they're tracked config, not disposable output; they
                           have their own `config/defaults/` "reset to default")

    `keep_videos=True` preserves only the Visualizer's generated videos (road/3D
    videos under `outputs/visualizer/road_videos/`, slow to regenerate) by moving
    them aside before the wipe and restoring them after. Deliberately does NOT
    preserve anything under `outputs/detection/` (including each sensor/crop's
    tracking-animation GIF) — a leftover file there would make the Home pipeline
    stepper's "has detection ever run" check see it and mark Detection & Tracking
    done, even though a reset should require re-running it.

    Returns a dict describing what happened, for a UI success message.
    """
    import shutil
    import tempfile

    had_derived = os.path.isdir(ds.derived_dir)
    had_outputs = os.path.isdir(ds.outputs_dir)

    preserved = []  # list of (staged_tmp_path, path_relative_to_outputs_dir)
    tmp_root = None
    if keep_videos and had_outputs and os.path.isdir(ds.road_videos_dir):
        tmp_root = tempfile.mkdtemp(prefix="wwd_keep_videos_")
        dst = os.path.join(tmp_root, "visualizer_road_videos")
        shutil.move(ds.road_videos_dir, dst)
        preserved.append((dst, os.path.join("visualizer", "road_videos")))

    if had_derived:
        shutil.rmtree(ds.derived_dir)
    if had_outputs:
        shutil.rmtree(ds.outputs_dir)

    for tmp_path, rel in preserved:
        dst = os.path.join(ds.outputs_dir, rel)
        os.makedirs(os.path.dirname(dst), exist_ok=True)
        shutil.move(tmp_path, dst)
    if tmp_root:
        shutil.rmtree(tmp_root, ignore_errors=True)

    return {"deleted_derived": had_derived, "deleted_outputs": had_outputs,
            "videos_kept": len(preserved)}
